...
class Penghargaan_pegawai extends CI_Controller
{
	...
	function get_ajax()
	{
		...
		foreach ($list as $pegawai) {
			...
			$row[] = '<a href="' . site_url('Member/penghargaan_pegawai/del/' . $pegawai->id_penghargaan_peg) . '/' . $pegawai->id_pegawai . '" onclick="return confirm(\'Yakin hapus data?\')"  class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> Hapus</a>';
			$data[] = $row;
		}
		$output = array(
			"draw" => @$_POST['draw'],
